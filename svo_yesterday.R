...
url<-"http://www.svo.aero/timetable/yesterday/"
...
indx<-grep("/timetable/yesterday",plain.text)
plain.text<-plain.text[indx[-1]]
...
for( i in 1:length(plain.text)){
  ...
  html_flight <- getURL(url_flight, followlocation = TRUE)
  ...
  print(i/length(plain.text)*100)
}
df <- data.frame(matrix(unlist(list_of_dop_info), nrow=length(plain.text), byrow=T))
yesterday<-cbind(svo,dest,df)
file<-as.character(paste("svo_yesterday",time,sep="_"))
file_name<-paste(file,"csv",sep=".")
write.csv(yesterday,file_name)
q(save="no")
########################